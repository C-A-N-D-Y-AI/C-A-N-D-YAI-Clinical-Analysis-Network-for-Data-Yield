...
export function MonthlyActivityChart({ data = [] }: MonthlyActivityChartProps) {
  // Encuentra el valor máximo base de 10 para que los valores pequeños no ocupen el 100%
    ...
    <div className="w-full bg-[#0D1525] border border-[#1A263D] rounded-3xl p-8 shadow-2xl h-full flex flex-col">
      ...
      <div className="relative flex-1 min-h-[250px] flex items-end justify-between gap-4 md:gap-8 pt-6 mt-auto">
        ...
        {data.map((item, index) => {
          ...
          // Si el valor es 0, le damos un 1% o 2% visual para que no quede vacío, si no, calculamos el porcentaje real
          const heightPercentage = item.value === 0 ? 2 : Math.max((item.value / maxValue) * 100, 4);
          
          // Cambiamos el color si es 0 para que se vea como una "ranura vacía"
          ...
          return (
            <div key={index} className="relative flex-1 flex flex-col items-center group h-full">
              <div className="w-full flex justify-center items-end h-full z-10 pb-8">
                ...
                >
                  {/* Tooltip siempre visible en hover */}
                  <span className="absolute -top-10 left-1/2 -translate-x-1/2 text-[11px] font-black text-white opacity-0 group-hover:opacity-100 transition-all bg-[#0D1525] px-2.5 py-1 rounded-md border border-white/10 z-20 shadow-xl scale-95 group-hover:scale-100">
